...
import {
  ...
  Upload, LogOut, Users, ShoppingCart, BarChart2, AlertCircle,
} from 'lucide-react'
...
function useBankData() {
  const [data, setData] = useState<BankDashboard | null>(() => {
    try { return loadDashboard() } catch { return null }
  })
  ...
  const importCSV = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (parsed.transactions.length === 0) throw new Error('Nenhuma transação encontrada.')
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900">
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {data && (
              <button onClick={clear} className="px-3 py-2 text-sm text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                Limpar
              </button>
            )}
            <button onClick={signOut} title="Sair" className="p-2 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!data && !loading && <EmptyState onFile={handleFile} />}
        {data && !loading && (
          activeTab === 'overview' ? (
            <div className="space-y-8">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <KpiCard title="Receita Total" value={formatCurrency(data.totalReceita)}
                  icon={<TrendingUp className="w-5 h-5 text-emerald-600" />} accent="emerald" />
                <KpiCard title="Despesa Total" value={formatCurrency(data.totalDespesa)}
                  icon={<TrendingDown className="w-5 h-5 text-orange-600" />} accent="orange" />
                <KpiCard
                ...
                <div className="h-[300px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.byMonth} margin={{ top: 4, right: 8, left: 8, bottom: 16 }}>
                      ...
                      <YAxis axisLine={false} tickLine={false} tickFormatter={v => `R$${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 11, fill: '#64748b' }} width={60} />
                      <Tooltip
                        contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
                        formatter={(value: number, name: string) => [formatCurrency(value), name === 'receita' ? 'Receita' : 'Despesa']}
                      />
                      <Legend formatter={name => name === 'receita' ? 'Receita' : 'Despesa'} />
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <RankingCard title="Principais Clientes" subtitle="Maiores fontes de receita"
                  icon={<Users className="w-5 h-5 text-emerald-600" />}
                  items={data.topClientes} total={data.totalReceita} accent="emerald" />
                <RankingCard title="Principais Fornecedores" subtitle="Maiores destinos de despesa"
                  icon={<ShoppingCart className="w-5 h-5 text-orange-600" />}
                  items={data.topFornecedores} total={data.totalDespesa} accent="orange" />
              </div>
            </div>
          ) : (
              ...
              <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {data.transactions
                      .filter(t => !txSearch || t.descricao.toLowerCase().includes(txSearch.toLowerCase()) || t.contraparte.toLowerCase().includes(txSearch.toLowerCase()))
                      .slice(0, 300)
                      .map(t => (
                        <tr key={t.id} className="hover:bg-slate-50/50">
                          <td className="px-4 py-2.5 text-slate-400 whitespace-nowrap">
                            {t.date instanceof Date ? t.date.toLocaleDateString('pt-BR') : String(t.date)}
                          </td>
                          <td className="px-4 py-2.5 text-slate-700 max-w-[180px] truncate font-medium">{t.contraparte}</td>
                          <td className="px-4 py-2.5 text-slate-500 max-w-[280px] truncate">{t.descricao}</td>
                          <td className={cn('px-4 py-2.5 text-right font-semibold whitespace-nowrap', t.tipo === 'receita' ? 'text-emerald-600' : 'text-red-500')}>
    ...
    <div className="flex flex-col items-center justify-center py-24 text-center space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm max-w-sm">
          Importe um CSV do Nubank para visualizar Receita, Despesa, Resultado, Clientes e Fornecedores.
        </p>
      </div>
...
function KpiCard({ title, value, subtitle, icon, accent }: {
  title: string; value: string; subtitle?: string; icon: ReactNode; accent: 'emerald' | 'orange' | 'blue' | 'red'
}) {
  const accentMap = {
  ...
  return (
    <div className={cn('bg-white p-5 rounded-2xl shadow-sm border-l-4 hover:shadow-md transition-all', accentMap[accent])}>
...
function RankingCard({ title, subtitle, icon, items, total, accent }: {
  title: string; subtitle: string; icon: ReactNode;
  items: { nome: string; total: number; count: number }[]; total: number; accent: 'emerald' | 'orange'
}) {
            ...
            <div key={item.nome}>
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2 min-w-0">
                  <span className={cn('text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full flex-shrink-0', badgeColor)}>{i + 1}</span>
